    ...
    if fastModeVar.get():
        ...
        if shoulderTypeVar.get() == "Fixed" :
            ...
            if noRealFunctionVar.get():
                #print("Всё оК!!!!!")
                plt, deviation, title, _ = smooth(x, y, y_real, yx, yx2, x2, x3, x4, func_lambd,compareWithNoised=compareWithNoisedVar.get(),func=func,table=table,plot1=plot1,shoulder_lengthh=findBestFixedShoulderWithoutRealFunction(x, y, y_real, yx, yx2, x2, x3, x4, func_lambd,plot21=plot21,plot22=plot22,showGraphs = showGraphs, compareWithNoised=compareWithNoisedVar.get(),dichotomyMin=int(minDichotomyVar.get()),dichotomyMax=int(maxDichotomyVar.get()),alg=fixedOptimalAlgVar.get(),savepath=savepath,table=table, smoothEdges=smoothVar.get(), deviationType=deviationTypeVar.get(), countEdges=сountEdgesVar.get(), realtime=realTimeVar.get(), progress=progress, window=secondFrame,shoulderLimit = int(shoulderLimitVar.get())), smoothEdges=smoothVar.get(), showRealFunction=showRealFuncVar.get(), countEdges=сountEdgesVar.get(), deviationType=deviationTypeVar.get())#realTimeVar.get())
            else:
                plt, deviation, title, _ = smooth(x, y, y_real, yx, yx2, x2, x3, x4, func_lambd,compareWithNoised=compareWithNoisedVar.get(),comparing=True,func=func,table=table,plot1=plot1,shoulder_lengthh=findBestShoulder(x, y, y_real, yx, yx2, x2, x3, x4, func_lambd,showGraphs = showGraphs, plot21=plot21, plot22=plot22,compareWithNoised=compareWithNoisedVar.get(),dichotomyMin=int(minDichotomyVar.get()),dichotomyMax=int(maxDichotomyVar.get()),alg=fixedOptimalAlgVar.get(),savepath=savepath,table=table, smoothEdges=smoothVar.get(), deviationType=deviationTypeVar.get(), countEdges=сountEdgesVar.get(), realtime=realTimeVar.get(), progress=progress, window=secondFrame,shoulderLimit = int(shoulderLimitVar.get())), smoothEdges=smoothVar.get(), showRealFunction=showRealFuncVar.get(), countEdges=сountEdgesVar.get(), deviationType=deviationTypeVar.get(),savepath=savepath)#realTimeVar.get())
...
noRealFunctionCheckbox = tk.Checkbutton(shoulderFrame, text="Не использовать настоящую функцию", variable=noRealFunctionVar)
...
generateWholeVectorCheckbox = tk.Checkbutton(optimizationFrame, text="Генерировать вектор целиком", variable=generateWholeVectorVar, command= lambda: toggleFrame(dynamicDichotomyButtonFrame,0 if generateWholeVectorVar.get() else 1))
...
radioLoop = tk.Radiobutton(optimizationTypeFrame, text="Перебор всех вариантов", variable=optimizationMethodVar, value="Loop",command=lambda: [toggleFrame(geneticFrame,0), generateWholeVectorCheckbox.configure(state="normal")])
radioLoop.pack(anchor=tk.W)
radioRandom = tk.Radiobutton(optimizationTypeFrame, text="Случайный перебор", variable=optimizationMethodVar, value="Random", command=lambda: [toggleFrame(geneticFrame,0), generateWholeVectorCheckbox.configure(state="normal")])
radioRandom.pack(anchor=tk.W)
dynamicDichotomyButtonFrame = tk.Frame(optimizationTypeFrame)
radioDynamicDichotomy = tk.Radiobutton(dynamicDichotomyButtonFrame, text="Дихотомия", variable=optimizationMethodVar, value="Dichotomy", command=lambda: [toggleFrame(geneticFrame,0), generateWholeVectorCheckbox.configure(state="disabled")])
radioDynamicDichotomy.pack(anchor=tk.W)
dynamicDichotomyButtonFrame.pack(anchor=tk.W)

radioGenetic = tk.Radiobutton(optimizationTypeFrame, text="Генетический алгоритм", variable=optimizationMethodVar, value="Genetic", command=lambda:[toggleFrame(geneticFrame,1), generateWholeVectorCheckbox.configure(state="normal")])
...
generatorFrame = tk.Frame(secondFrame, borderwidth=0.5, relief="solid")
...
queueButton = tk.Button(generatorFrame, text="Запустить очередь из file.txt", command=onQueueButtonClick)
queueButton.pack()


secondaryGraphFrame = tk.Frame(generatorFrame,borderwidth=0.5, relief="solid")
fig2 = mpl.figure.Figure(figsize=(4, 6), dpi=100)
fig2.tight_layout()
plot21 = fig2.add_subplot(211)
plot22 = fig2.add_subplot(212)
...
#canvas2.draw()
#canvas2.get_tk_widget().pack()
secondaryGraphFrame.pack()

generatorFrame.pack(side="left",padx=5, pady=5,anchor=tk.N)
...
tableFrame = tk.Frame(secondFrame,borderwidth=0.5, relief="solid")
...
graphFrame = tk.Frame(secondFrame,borderwidth=0.5, relief="solid")
fig1 = mpl.figure.Figure(figsize=(9.9, 9.42), dpi=100)
fig1.tight_layout()
