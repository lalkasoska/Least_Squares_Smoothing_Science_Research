...
def smooth(x, y, y_real, yx, yx2, x2, x3, x4, func_lambd, table=None, path='dots.xlsx', shoulder_lengthh=10,
    ...
    for i in range(0, n):
        ...
        if not ((i == 0 or i == n - 1) and sideShoulderType == "Increment"):
            ...
            if realtime:
                if abs(max(y[parabola_start:parabola_end + 1])) > 2 or abs(min(y[parabola_start:parabola_end + 1])) > 2:
                    color = 'deepskyblue'
                elif i>=shoulder_length and n - i - 1 >= shoulder_length:
                    color = 'darkblue'
                else:
                    color = 'green'
                plt.clf()
                ...
                if i != len(x)-1:
                    ...
                    plt.plot(xspace10, fragment_smoothed, color, label='Сглаживающая парабола', linewidth=3)
                # plt.plot(xspace,y_real, 'g', label='real_function')
                if linePlot:
                    plt.plot(x[:min(shoulder_length,i)], y_smoothed[:min(shoulder_length,i)], 'green', label='Сглаженные края', linewidth=2)
                else:
                    plt.plot(x[:min(shoulder_length,i)], y_smoothed[:min(shoulder_length,i)], 'green', linestyle="", marker='.', label='Сглаженные края')
                if n-i-1 >= shoulder_length:
                    if linePlot:
                        plt.plot(x[shoulder_length:i + 1], y_smoothed[shoulder_length:i + 1], 'b', label='Сглаженные точки', linewidth=2)
                    else:
                        plt.plot(x[shoulder_length:i + 1], y_smoothed[shoulder_length:i + 1], 'bo', marker='.', label='Сглаженные точки')
                else:
                    if linePlot:
                        plt.plot(x[shoulder_length:n-i-1], y_smoothed[shoulder_length:n-i-1], 'b', label='Сглаженные точки', linewidth=2)
                        plt.plot(x[n - i - 1:], y_smoothed[n - i - 1:], 'green',
                                  linewidth=2)
                    else:
                        plt.plot(x[shoulder_length:i], y_smoothed[shoulder_length:i], 'bo', marker='.', label='Сглаженные точки')
                        plt.plot(x[n-shoulder_length:i], y_smoothed[n-shoulder_length:i], 'green', linestyle="",marker='.'
                                 )
                if i > 165-shoulder_length:
                    plt.plot(x[165-shoulder_length:min(i,165+shoulder_length+1)], y_smoothed[165-shoulder_length:min(i,165+shoulder_length+1)],  'deepskyblue',linestyle="", marker='.',
                             label='Области выбросов')
                if i > 248-shoulder_length:
                    plt.plot(x[248 - shoulder_length:min(i, 248 + shoulder_length+1)],
                             y_smoothed[248 - shoulder_length:min(i, 248 + shoulder_length+1)], 'deepskyblue',linestyle="", marker='.')
                if i != len(x)-1:
    ...
    if comparing:
        ...
        for i in range(len(y)):
            deviationyLOWESS += (y_real[i] - yLOWESS[i]) ** 2
        for i in range(len(y)):
            deviationyLOESS += (y_real[i] - yLOESS[i]) ** 2
    if countEdges:
    ...
    if comparing:
        ...
        print(f"deviationyLOWESS deviation: {deviationyLOWESS}")
        print(f"deviationyLOESS deviation: {deviationyLOESS}")
        ...
        with open(filedialog.asksaveasfilename(initialdir="SuperComparison\Fix") + ".txt", 'w+') as f:
            f.write(f"MyFunc deviation: {deviation}\n"
                    ...
                    f"LOWESS deviation: {deviationyLOWESS}\n\n"
                    f"LOESS deviation: {deviationyLOESS}\n\n")
    ...
    if plot1 is not None:
        ...
        if comparing:
            ...
            plot1.plot(x, yLOWESS, color='red',linestyle="--", label='LOWESS',linewidth=3.0)
            plot1.plot(x, yLOESS, color='black',linestyle="--", label='LOESS',linewidth=3.0)
...
def smoothSeparate(x, y, y_real, yx, yx2, x2, x3, x4, func_lambd, shoulderVector, smoothEdges=False,
    ...
    for i in range(0, len(shoulderVector)):
        ...
        if realtime:
            ...
            if showRealFunction:
                y_realDrawn = [func_lambd(xi) for xi in xspace]
                plt.plot(xspace, y_realDrawn, 'g', label='Real function')
            if i != len(x) - 1:
                plt.plot(x[parabola_start:parabola_end + 1], y[parabola_start:parabola_end + 1],
                         'ro', alpha=0.4, label="Точки опоры")
    ...
    if comparing:
        ...
        for i in range(len(y)):
            deviationyLOESS += (y_real[i] - yLOESS[i]) ** 2
        for i in range(len(y)):
            deviationySavGol += (y_real[i] - ySavGol[i]) ** 2
    ...
    if comparing:
        ...
        with open(filedialog.asksaveasfilename(initialdir="SuperComparison\Fix") + ".txt", 'w+') as f:
            f.write(f"MyFunc deviation: {deviation}\n"
                    ...
                    f"LOWESS deviation: {deviationyLOWESS}\n\n"
                    f"LOESS deviation: {deviationyLOESS}\n\n"
                    f"SavGol deviation: {deviationyLOESS}\n\n")
    ...
    if plot1 is not None:
        ...
        if comparing:
            ...
            plot1.plot(x, yMLP, color='orange', label='Neural Network')
            plot1.plot(x, yNeighbours, color='purple', label='k Nearest Neighbours')
            plot1.plot(x, yLOWESS, color='gray', linestyle="--", label='LOWESS', linewidth=3.0)
            plot1.plot(x, yLOESS, color='m', linestyle="--", label='LOESS', linewidth=3.0)
            plot1.plot(x, ySavGol, color='black', linestyle="--", label='SavGol', linewidth=3.0)
        plot1.plot(x, y, 'ro',alpha=0.2, label='Точки с шумом')
        if showRealFunction:
            y_realDrawn = [func_lambd(xi) for xi in xspace]
        ...
        # plt.plot(xspace10,fragment_smoothed, 'b', label='Parabola')
        if linePlot:
            plot1.plot(x, y_smoothed, 'b', label='Модификация', linewidth=4)
        else:
            plot1.plot(x, y_smoothed, 'bo', marker='.', label='Модификация')
        plot1.legend(loc='upper left')
...
def deviationForOneDot(shoulder_length, index, x, y, y_real, yx, yx2, x2, x3, x4, deviationType):
    ...
    if shoulder_length > min(index, len(x) - index - 1):
        deviation = float("inf")#(y[index] - y_real[index])**2 * 10 * min(index, len(x) - index - 1)
...
def findBestFixedShoulderWithoutRealFunction(x, y, y_real, yx, yx2, x2, x3, x4, func_lambd, smoothEdges=True,
    ...
    if alg == "Loop":
        ...
        for i in range(len(derivatives)):
            if not startFlat:
                if sum(derivatives[i - 1:i + 2]) / 3 < 0.5:
                    startFlat = i
            elif sum(derivatives[i - 1:i + 2]) / 3 > 0.5 or i + 1 > len(derivatives):
                endFlat = i
                break
        bestShoulder = round(endFlat * 0.5 + startFlat * 0.5)
    ...
    if compareWithNoised:
        # plt.cla()
        axs[0].set_title("График производных")
